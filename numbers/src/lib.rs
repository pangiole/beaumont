...
/// - either use custom literals, such as `1234.56d` or `"-234.00"d`,
/// - or invoke the [`Decimal::from`] converter functions,
/// - or invoke the [`Decimal::new`] factory function
///
/// ```rust
...
/// ```
///
/// Once created, you can apply operations such as negation, additions, multiplication, etc.
///
/// ```rust
...
///
/// # Memory
/// To make them perform well and look like built-in numerical types, our decimal numbers are:
///
/// - immutable (cannot be modified after creation),
/// - having fixed size and therefore stack-allocated (unless explicitly boxed),
...
/// The maximum coefficient allowed in representing decimal numbers
pub const MAX_COEFFICIENT: i32 = i32::MAX;

/// The minimum coefficient allowed in representing decimal numbers
pub const MIN_COEFFICIENT: i32 = i32::MIN;

/// The maximum scaling allowed in representing decimal numbers
pub const MAX_SCALING: u8 = 8;

/// The minimum scaling allowed in representing decimal numbers
pub const MIN_SCALING: u8 = 0;

/// The maximum precision possible in representing decimal numbers
...
impl fmt::Display for DecimalError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            DecimalError::BadFormat(msg) => write!(f, "{msg}"),
            DecimalError::ScalingOverflow => write!(f, "Scaling overflow"),
            DecimalError::CoefficientOverflow => write!(f, "Coefficient overflow")
        }
    }
}


/// Rounding mode to be used in decimal number operations requiring rounding
...
impl Decimal {
    /// Same as the [`Decimal::try_new`] function, but it panics instead of resulting [`DecimalError`]
    ...
    /// # Examples
    /// ```rust
    /// # use beaumont_numbers::{Decimal, DecimalError, MAX_SCALING};
    ...
    /// assert!(d2.is_err());
    /// assert!(matches!(d2.unwrap_err(), DecimalError::ScalingOverflow));
    /// ```
    pub fn try_new(coefficient: i32, scaling: u8) -> Result<Self, DecimalError> {
        if scaling > MAX_SCALING {
            return Err(DecimalError::ScalingOverflow);
        }
    ...
    // TODO Avoid .to_string() wherever possible (event in docs and tests)
}
