...
// Inner implementation of the vector macro
mod vector;
...
// Inner implementation of the matrix macro
mod matrix;
...
pub fn matrix(items: TokenStream) -> TokenStream {
    matrix::expand(parse_macro_input!(items))
        .unwrap_or_else(syn::Error::into_compile_error)
        .into()
    // "Matrix { components: Box::new([1, 2, 3, 4, 5, 6]), rows: 2, columns: 3}".parse::<TokenStream>().unwrap()
}